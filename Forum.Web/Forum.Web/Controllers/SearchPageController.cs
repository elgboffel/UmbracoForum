    ...
    public class SearchPageController : RenderMvcController
    {
		...
		public ActionResult Index(RenderModel renderModel, string q = "", int p = 1)
		{
			var model = new SearchModel(renderModel.Content, renderModel.CurrentCulture)
			{
				CurrentPage = p,
				SearchTerm = q,
				PageSize = renderModel.Content.GetPropertyValue<int>("pageSize", 10),
				...
				SearchFields = new List<string> { "nodeName", "heading", "bodyText", "pageBodyText", "pageLayout", "quoteText", "teaserHeading", "teaserDescription", "FileTextContent" },
				PreviewFields = new List<string> { "bodyText", "pageBodyText", "pageLayout", "teaserDescription", "FileTextContent" },
				PreviewLength = 150,
			};
			...
			if (!string.IsNullOrWhiteSpace(model.SearchTerm))
			{
				...
				results.AddRange(SearchEmployees(model));
		...
		private ISearchResults SearchEmployees(SearchModel model)
		{
			...
			string[] fields = new string[] { "nodeName", "firstName", "lastName", "keyQualifications", "shortDescription", "address", "initials", "title", "education" };
			var criteria = searcher.CreateSearchCriteria();
			...
			LogHelper.Info<SearchPageController>("Employees: {0}", () => query.ToString());
